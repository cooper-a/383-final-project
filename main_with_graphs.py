...
def plot_height_over_time(frames):

    figure, axis = plt.subplots()
    figure.suptitle("Water Height vs Time")
    axis.set_xlabel("Time (s)")
    axis.set_ylabel("Water Height (m)")
    ...
    for df in frames:
        ...
        if df[2]:
            label = f"{df[1]}m tube with T-joint"
        else:
            label = f"{df[1]}m tube with no T-joint"
    ...
    for tube_length_with_t_bool, experimental_time in zip(TUBE_LENGTHS_WITH_T_BOOLS, EXPERIMENTAL_RESULTS):
        ...
        with Loader("Water is draining..."):
            while water_height >= (END_HEIGHT + tube_length * SIN_THETA):
                ...
                data.append([total_time, v1, v2, water_height, reynolds_number])
